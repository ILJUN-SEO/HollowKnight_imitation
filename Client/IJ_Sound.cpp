...
namespace IJ
{
	...
	bool Sound::LoadWavFile(const std::wstring& path)
	{
		...
		memset(&myBufferDesc, 0, sizeof(DSBUFFERDESC));
		myBufferDesc.dwBufferBytes = pChild.cksize;
		myBufferDesc.dwSize = sizeof(DSBUFFERDESC);
		myBufferDesc.dwFlags = DSBCAPS_STATIC | DSBCAPS_LOCSOFTWARE | DSBCAPS_CTRLVOLUME;
		myBufferDesc.lpwfxFormat = &wft;

		if (FAILED(SoundManager::GetDevice()->CreateSoundBuffer(&myBufferDesc, &mySoundBuffer, NULL)))
		{
		...
		mySoundBuffer->Lock(0, pChild.cksize, &pWrite1, &dwlength1
		...
		mySoundBuffer->Unlock(pWrite1, dwlength1, pWrite2, dwlength2);
	...
	void Sound::SetPosition(float position, bool loop)
	{
		...
		DWORD dwBytes = (DWORD)((position / 100.f) * (float)myBufferDesc.dwBufferBytes);
		mySoundBuffer->SetCurrentPosition(dwBytes);
	...
	{
		myVolume = GetDecibel(volume);
		mySoundBuffer->SetVolume(myVolume);
	}
